    ...
    public static Specification<Book> withFilters(
        ...
        return (root, query, cb) -> {
            ...
            if (author != null && !author.isBlank())
                ps.add(cb.like(cb.lower(root.get("author")), "%" + author.toLowerCase() + "%"));
            if (category != null)
                ps.add(cb.equal(root.get("category"), category));
            if (publishedYear != null)
                ps.add(cb.equal(root.get("publishedYear"), publishedYear));

            if (Boolean.TRUE.equals(available)) {
                // Books with at least one AVAILABLE copy
                Subquery<Long> sub = query.subquery(Long.class);
                Root<BookCopy> cr = sub.from(BookCopy.class);
                sub.select(cb.count(cr)).where(cb.and(
                        cb.equal(cr.get("book"), root),
                        cb.equal(cr.get("status"), CopyStatus.AVAILABLE)
                ));
